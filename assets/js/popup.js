...
//template to go to script and get back jquery
// function btntest() {
//   chrome.tabs.query({active: true, currentWindow: true}, demopop);
//   function demopop(tabs) {
...
chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
  chrome.tabs.sendMessage(tabs[0].id, {function: "sitecheck"}, function(response) {
...
function chips(authSel) {
  if (authSel != '') {
    if (authSel.indexOf(',') == -1) {
      $('.placeAuth').html('<div class="chip"><span id="chip_'+authSel+'">'+authSel+'</span><i id="i_'+authSel+'" class="close material-icons chips_close">close</i></div>');
    } else {
      $('.placeAuth').html('');
      var authSelArr = authSel.split(',');
      $(authSelArr).each(function() {
        authSel = this;
        $('.placeAuth').append('<div class="chip"><span id="chip_'+authSel+'">'+authSel+'</span><i id="i_'+authSel+'" class="close material-icons chips_close">close</i></div>');
      });
    }
    ...
    $('.placeAuth').html('<p>&nbsp;&nbsp;&nbsp; None -- Click below</p>');
  }
}
...
function authDefaults() {
  ...
  $('#currAuthType').text('ip,uid');
  chips('ip,uid');
}

//get any information stored
chrome.storage.local.get(['sitestorage','credsstorage','uisite','permalink','saveAuthOps'], function (items) {
...
function getsite() {
  if(site == 'eadmin') {
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
      chrome.tabs.sendMessage(tabs[0].id, {function: "getsite"}, function(response) {
        if (typeof response !== 'undefined') {
...
function getup() {
  if(site == 'eadmin') {
    ...
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
      chrome.tabs.sendMessage(tabs[0].id, {function: "getup",custId: custId,groupId: groupId}, function(response) {
...
function getuisite() {
  if(site == 'ebscohost') {
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
      chrome.tabs.sendMessage(tabs[0].id, {function: "getuisite"}, function(response) {
...
function createlink() {
  ...
  if ($('#embed_logins').is(":checked")) {
    user = $('#showUser').text();
    pwd = $('#showPass').text();
    if (user == '' && pwd == '') { var $toastContent = $('<span>Missing User/Password</span>'); M.toast({html: $toastContent}, 2000); }
    authtype = 'uid';
  ...
  profURL = 'https://search.ebscohost.com/login.aspx?custid='+custId+'&groupid='+groupId+'&profid='+profId+'&authtype='+authtype+creds;
  $('#showUrl').text(profURL);
...
$('.switch input').on('click', function() {
  ...
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    chrome.tabs.sendMessage(tabs[0].id, {function: "adminops",option:adminOpsArr});
  });
});
...
$('a[href="#authtype"]').on('click', function() {
  ...
  if (!currAuthType.match(/(uid|guest|cpid|shib|sso|custiud|athens|embed_logins)/g)) {
...
$('input[name="authtype"]').on('click',function() {
  ...
  if(this.value == 'embed_logins') {
    ...
    if (currAuthType.match(/(ip|cookie|url)/g)) {
      if (this.checked === true && this.type == 'checkbox' && currAuthType.indexOf(this.value) == -1) {
...
function searchcid() {
  custid = $('#showCust').text();
  var searchCust = 'http://cust.ebsco-gss.net/search.rb?q='+custid+'&p=simple';
  var win = window.open(searchCust, '_blank');
  if (win) {
    win.focus();
  } else {
    alert('Please allow popups for this website');
...
$('#submit').on('click',function(){
  if($('#name').val() !== '' && $('#email').val() !== '' && $('#textarea1').val() !== '' && $('#email').val().indexOf('@') > -1) {
    $('#mail').css('display','none');
    $('#success').css('display','block');
  } else {
    alert('Error: Missing a field. If you\'re still having issues, email: cmcinnis@ebsco.com');
  }
});

// function clickAuthSelect() {
//   custid = $('#showCust').text();
//   chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
//     chrome.tabs.sendMessage(tabs[0].id, {function: "clickAuthSelect"});
//   });
// }

// document.getElementById("clickAuthSelect").onclick = clickAuthSelect;
...
document.getElementById("searchcid").onclick = searchcid;
// document.getElementById("credPgDown").onclick = credPgDown;
